...
def greedy(inicial, objetivo):
    fila = []
    heapq.heappush(fila, (manhattan(inicial, objetivo), inicial, []))
    visitados = set()
    nos_expandidos = 0

    while fila:
        heuristica, atual, caminho = heapq.heappop(fila)
        ...
        for sucessor in gerar_sucessores(atual):
            ...
            heuristica_nova = manhattan(sucessor, objetivo)
            heapq.heappush(fila, (heuristica_nova, sucessor, novo_caminho))
